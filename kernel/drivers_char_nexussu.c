...
#define NEXUSSU_REVOKE_UID _IOW(NEXUSSU_IOC_MAGIC, 4, uid_t) // NEW
...
static long nexussu_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case NEXUSSU_REVOKE_UID: // NEW
            if (copy_from_user(&target_uid, (uid_t __user *)arg, sizeof(uid_t))) return -EFAULT;
            nexussu_revoke_trusted_uid(target_uid);
            printk(KERN_INFO "NexusSU: Revoked UID %d\n", target_uid);
            break;
